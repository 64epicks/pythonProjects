...
class Network:
    ...
    def updateParameters(self, batch, lr):
        ...
        self.weights1d = [w-(lr/len(batch))*nw
                        for w, nw in zip(self.weights1d, weightDerivative)]
        self.biases1d = [b-(lr/len(batch))*nb
                       for b, nb in zip(self.biases1d, biasDerivative)]
    ...
    def backprop(self, x, y):
        ...
        for l in xrange(2, self.num_layers):
            ...
            delta = np.dot(self.weights1d[-l+1].transpose(), delta) * sp
            der_b[-l] = delta
